...
public class PlayerAiming : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            isAiming = true;
            if (currentReticle != null) currentReticle.SetActive(true);
            UpdateReticlePosition();
        }
        else if (Input.GetMouseButtonUp(1))
        {
            isAiming = false;
            if (currentReticle != null) currentReticle.SetActive(false);
        }
    }
    ...
    void LateUpdate()
    {
        if (isAiming)
        {
            ...
            if (!isMoving)
            {
                ...
                if (directionToMouse > 0.1f) scale.x = Mathf.Abs(scale.x);
                else if (directionToMouse < -0.1f) scale.x = -Mathf.Abs(scale.x);
    ...
    private void UpdateReticlePosition()
    {
        ...
        targetAimPosition = (hit.collider != null) ? (Vector3)hit.point : desiredPos;
    ...
    public Vector3 GetAimPosition() => targetAimPosition;
